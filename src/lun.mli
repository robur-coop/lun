(** {1 Optics in OCaml: Lun(ette).}
    ...
    records, the second for ADTs. These views offer values of a certain type [t]
    which can subsequently be composed together. These values describe your type
    and they can be self-generated by a {b ppx}.
    ...
    {[
    let update_c s v = { s with b = { s.t with c = v } }
    ]}
    ...
    {[
    type t = Foo of int * int | Bar of (int * int)

    let foo () =
      Lun.prism (fun (a, b) -> Foo (a, b)) @@ function
      | Foo (a, b) -> Ok (a, b)
      | x -> Error x

    let bar () =
      Lun.prism (fun v -> Bar v) @@ function Bar v -> Ok v | x -> Error x
    ]}
    ...
    {{:https://v2.ocaml.org/manual/polymorphism.html}Polymorphism and its
     limitations.}
    ...
    {[
    let hd () =
      Lun.prism (fun v -> [ v ]) @@ function v :: _ -> Ok v | v -> Error v
    ]}
...
(** [get optic s] tries to {i project} ['a] from ['s]. If the given [optic] is a
    {!val:lense}, this function {b never} fails. Otherwise, if the given [optic]
    is a {!val:prism}, it can raise an exception {!exception:Undefined}. For
    instance:

    {[
    type t = A | B

    let a () = prism (fun () -> A) @@ function A -> Ok () | x -> Error x

    let () =
      match Lun.get a B with
